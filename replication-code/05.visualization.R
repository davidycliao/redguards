...
network_distance <- redguard_estimates[redguard_estimates$activist %in% reps,  c("id_doc", "activist", "x", "fact_eng")]

network_distance["activist_eng"] <- 
  c( "聂元梓 Nie Yuanzi", 
...
A <- list()
X <- list()
D <- network_distance$x

for (i in 1:length(D)) {
  A[[i]] <- rep(network_distance[i,"activist_eng"] ,length(D))
  X[[i]] <- rep(network_distance[i,"x"], length(D))
}

A <- unlist(A)
X <- unlist(X)
DA <- rep(network_distance$activist_eng ,length(D))
DX <- rep(network_distance$x ,length(D))

P <- matrix(nrow = length(A), ncol = 3)
...
layout["fact_eng"] <- ifelse( layout$name %in% network_distance$activist_eng, network_distance$fact_eng, NA)
...
network_plot <- ggraph(layout) +
  ...
  geom_node_text(aes(label = name,size = `Centrality \n in Ideal Points`), 
                 size = 3, repel = FALSE, family = "STHeiti", col = "black", fontface = "bold") +  
